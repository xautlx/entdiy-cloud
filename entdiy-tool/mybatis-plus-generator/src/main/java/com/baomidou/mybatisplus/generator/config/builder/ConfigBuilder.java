...
public class ConfigBuilder {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        if (null != configPathInfo) {
            ...
            setPathInfo(pathInfo, template.getDtoList(), outputDir, ConstVal.DTO_PATH, ConstVal.DTO);
            setPathInfo(pathInfo, template.getDtoEdit(), outputDir, ConstVal.DTO_PATH, ConstVal.DTO);
        }
    }
    ...
    private void processTypes(StrategyConfig config) {
        ...
        if (strategyConfig.getSuperClassStrategy() != null) {
            SuperClassStrategy superClassStrategyInstance = strategyConfig.getSuperClassStrategy().getDeclaredConstructor().newInstance();
            Class<?>[] superEntityClassList = superClassStrategyInstance.candidateSuperEntityClass();
            for (Class<?> superEntityClass : superEntityClassList) {
                strategyConfig.setSuperEntityClass(superEntityClass, NamingStrategy.underline_to_camel);
            }
        }
    }
    ...
    private void checkImportPackages(TableInfo tableInfo) {
        if (strategyConfig.getSuperClassStrategy() != null) {
            SuperClassStrategy superClassStrategyInstance = strategyConfig.getSuperClassStrategy().getDeclaredConstructor().newInstance();
            Class<?> superEntityClass = superClassStrategyInstance.parseSuperEntityClass(tableInfo);
            String superEntityClassName = superEntityClass.getCanonicalName();
            tableInfo.setSuperEntityClass(superEntityClassName);
            tableInfo.getImportPackages().add(superEntityClassName);

            Class<?> superListDtoClass = superClassStrategyInstance.parseSuperListDtoClass(tableInfo);
            String superListDtoClassName = superListDtoClass.getCanonicalName();
            tableInfo.setSuperListDtoClass(superListDtoClassName);
            tableInfo.getImportListDtoPackages().add(superListDtoClassName);

            Class<?> superEditDtoClass = superClassStrategyInstance.parseSuperEditDtoClass(tableInfo);
            String superEditDtoClassName = superEditDtoClass.getCanonicalName();
            tableInfo.setSuperEditDtoClass(superEditDtoClassName);
            tableInfo.getImportEditDtoPackages().add(superEditDtoClassName);
